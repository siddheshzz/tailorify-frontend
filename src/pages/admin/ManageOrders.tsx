...
export const ManageOrders: React.FC = () => {
  ...
  const [deletingOrder, setDeletingOrder] = useState<Order | null>(null);
  const [editingOrder, setEditingOrder] = useState<Order | null>(null);
  const [viewingImages, setViewingImages] = useState<Order | null>(null);
  ...
  const handleUpdate = async (data: Partial<OrderCreate>) => {
  ...
  return (
    <AdminLayout>
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => setViewingImages(order)}
                    title="View Images"
                  >
                    <ImageIcon size={16} />
                  </Button>
      ...
      <Modal
        ...
        title={`Edit Order #${editingOrder?.id.slice(0, 8)}`}
        size="lg"
      >
        {editingOrder && (
          <AdminOrderForm
